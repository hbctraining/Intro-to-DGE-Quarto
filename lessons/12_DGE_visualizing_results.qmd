...
```{r}
#| label: view_counts_code
#| eval: false
# View the tibble to make sure it looks correct
normalized_counts %>% head() %>% View()
```

```{r}
#| label: view_counts_render
#| echo: false
# View the tibble to make sure it looks correct
normalized_counts %>% head() %>% DT::datatable()
```
...
```{r}
#| label: add_gene_names_alternative
normalized_counts <- counts(dds, normalized = TRUE) %>% 
  data.frame() %>%
  rownames_to_column(var = "gene") %>%
  as_tibble() %>%
...
As discussed in the [Set up lesson](03_DGE_setup_and_overview.qmd#review-of-the-dataset), knockdown of Mov10 was achieved using siRNA. RNA interference is inefficient: some RNAs escape degradation, and thus some expression remains.
:::

```{r}
#| label: plot_one_gene_ggplot
...
If you are interested in plotting the expression of multiple genes all together, please refer to [the short lesson linked here](Aside_top20_genes-expression_plotting.qmd) where we demo this for the top 20 most significantly expressed genes.
:::
...
```{r}
#| label: label_top_DEG_code
#| eval: false
# Look at the new table
res_tableOE_tb %>% head(n = 15) %>% View()
```

```{r}
#| label: label_top_DEG_render
#| echo: false
# Look at the new table
res_tableOE_tb %>% head(n = 15) %>% DT::datatable()
```
...
[Next Lesson](13_DGE_LRT_results.qmd)
