...
description: |
  This lesson provides an overview of genomic annotation resources and demonstrates practical strategies for retrieving gene-level information to support RNA-seq and differential expression analyses. Participants will be introduced to various types of annotation databases (such as Ensembl, NCBI, Gene Ontology, and KEGG), consider the importance of genome builds and compare Bioconductor packages for querying and managing gene annotations. Participants will use AnnotationHub and ensembldb to facilitate gene ID conversion, annotation data extraction and techniques for integrating annotations with differential gene expression results.
author:
  - Meeta Mistry
  - Radhika Khetani
  - Mary Piper
...
While AnnotationDbi is a popular tool, **we will not be walking through code to use this package**. However, if you are interested in more detail, we have [materials linked](Aside_AnnotationDbi_lesson.qmd) here with examples using our current dataset.
...
unique(ah$dataprovider) %>% head(n = 20)
```
...
```{r}
#| label: AH_human_genes_code
#| eval: false
# Extract gene-level information
genes(human_ens, return.type = "data.frame") %>% head() %>% View()
```

```{r}
#| label: AH_human_genes_render
#| echo: false
# Extract gene-level information
genes(human_ens, return.type = "data.frame") %>% head() %>% DT::datatable()
```

But note that it is just as easy to get the transcript- or exon-level information:

```{r}
#| label: AH_human_tx_code
#| eval: false
# Extract transcript-level information
transcripts(human_ens, return.type = "data.frame") %>% head() %>% View()
```

```{r}
#| label: AH_human_tx_render
#| echo: false
# Extract transcript-level information
transcripts(human_ens, return.type = "data.frame") %>% head() %>% DT::datatable()
```

```{r}
#| label: AH_human_ex_code
#| eval: false
# Extract exon-level information
exons(human_ens, return.type = "data.frame") %>% head() %>% View()
```

```{r}
#| label: AH_human_ex_render
#| echo: false
# Extract exon-level information
exons(human_ens, return.type = "data.frame") %>% head() %>% DT::datatable()
```
...
Finally, it would be good to know **what proportion of the Ensembl identifiers do not map to an Entrez identifier**:
...
That's almost half of our genes! If we plan on using Entrez ID results for downstream analysis, we should definitely keep this in mind. If you look at some of the Ensembl IDs from our query that returned `NA`, these map to pseudogenes (e.g., [ENSG00000265439](https://useast.ensembl.org/Homo_sapiens/Gene/Summary?g=ENSG00000265439;r=6:44209766-44210063;t=ENST00000580735)) or non-coding RNAs (e.g., [ENSG00000265425](http://useast.ensembl.org/Homo_sapiens/Gene/Summary?g=ENSG00000265425;r=18:68427030-68436918;t=ENST00000577835)). The discrepancy (which we can expect to observe) between databases is due to the fact that each implements its own different computational approaches for generating the gene builds.
...
[Next Lesson](16_FA_over-representation_analysis.qmd)
