...
description: |
  This lesson introduces functional analysis for RNA-seq data, focusing on over-representation analysis (ORA) using Gene Ontology (GO) terms and other annotation sources. Participants will explore the principles and statistical theory behind functional enrichment, including the hypergeometric test, apply ORA using tools like clusterProfiler and interpret functional enrichment results in the context of biological hypotheses. The lesson covers the creation of gene and background lists, enrichment, visualization and strategies for critical interpretation of functional analysis output in RNA-seq experiments.
author:
  - Meeta Mistry
  - Radhika Khetani
  - Mary Piper
...
::: callout-note
If you were unable to generate the `annotations_ahb` object, you can download the annotations to your `data` folder by right-clicking [here](./data/annotations_ahb.csv) and selecting "Save link as..."
...
The different organisms with annotation databases available to use with for the `OrgDb` argument can be found [here](../img/orgdb_annotation_databases.png).
...
```{r}
#| label: enrichGO_render
#| echo: false
# View results (prevent "geneID" column from wrapping so the table is not incredibly long)
cluster_summary %>% DT::datatable() %>% DT::formatStyle("geneID", "white-space" = "nowrap")
```
...
cnetplot(ego2, 
         categorySize = ego2@result$pvalue, 
         foldChange = OE_foldchanges, 
         showCategory = 5)
```
...
[Next Lesson](17_FA_functional_class_scoring.qmd)
