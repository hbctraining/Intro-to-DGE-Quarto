...
description: |
  This lesson introduces functional class scoring approaches for functional enrichment analysis in RNA-seq experiments, emphasizing Gene Set Enrichment Analysis (GSEA). Participants will explore the rationale behind functional class scoring, perform GSEA with GO and KEGG gene sets using clusterProfiler and interpret pathway-level results when only modest gene-level changes are observed. The lesson also covers available resources for advanced enrichment and network-based analyses and discusses complementary approaches such as co-expression clustering to identify patterns of functional gene modules.
author:
  - Meeta Mistry
  - Radhika Khetani
  - Mary Piper
...
One commonly used tool that is classified under Functional class scoring (FCS) is [Gene Set Enrichment Analysis (GSEA)](https://www.pnas.org/content/102/43/15545). Gene set enrichment analysis utilizes the gene-level statistics or log2 fold changes for all genes to look to see whether gene sets for particular biological pathways are enriched among the large positive or negative fold changes. 

<p align="center"> 
<img src="../img/gsea_theory.png" width="600">
</p> 
  
Gene sets are pre-defined groups of genes that are functionally related. Commonly used gene sets include those derived from KEGG pathways, Gene Ontology terms, MSigDB, Reactome, or gene groups that share some other functional annotations, etc. [[1](../../resources/pathway_tools.pdf)]. 
...
```{r}
#| label: gsea_results_code
#| eval: false
# Look at GSEA results
gseaKEGG_results %>% View()
```

```{r}
#| label: gsea_results_render
#| echo: false
# Look at GSEA results (prevent "core_enrichment" column from wrapping so the table is not incredibly long)
gseaKEGG_results %>% DT::datatable() %>% DT::formatStyle("core_enrichment", "white-space" = "nowrap")
```
...
[Next Lesson](18_DGE_summarizing_workflow.qmd)
