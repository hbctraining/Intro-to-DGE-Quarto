...
title: "AnnotationDbi"
description: |
  This lesson introduces the AnnotationDbi package and demonstrates how to use it to access and query annotation databases for gene identifier conversion and functional data retrieval. It covers the use of organism-specific databases such as `org.Hs.eg.db` and `EnsDb.Hsapiens.v86`, illustrates key functions (like select, columns, keys, and keytypes) and discusses strategies for handling database mapping issues, such as missing and duplicate annotation results, in RNA-seq and differential expression analyses.
author:
  - Meeta Mistry
  - Radhika Khetani
  - Mary Piper
...
```{r}
#| label: orgHs_symbols_remove_duplicates
...
```{r}
#| label: EnsDb_symbols_remove_duplicates
# Determine the indices for the non-duplicated genes
