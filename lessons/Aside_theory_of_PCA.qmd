...
title: "Theory of PCA"
description: |
  This lesson introduces the mathematical foundations and step-by-step implementation of Principal Components Analysis (PCA). Participants will understand how PCA is used to reduce the dimensionality of high-throughput datasets, such as those produced by RNA-seq experiments, to explore data variance and structure. The lesson covers the derivation of covariance matrices, calculation of eigenvectors and eigenvalues, transformation to principal component space and comparison between manual and the `prcomp()` function's results. 
author: 
  - Will Gammerdinger
  - Noor Sohail
date: "2025-07-22"
...
We are going to discuss the steps for deriving a Principal Components analysis, but in order to do it, we are going to use an example dataset. Let's start by creating an tibble that has gene expression values for two genes from four samples.
...
ggplot(expression_tibble, aes(x = Gene_A, y = Gene_B, label = samples)) +
  ...
  xlab("Gene A") +
  ylab("Gene B") +
  ggtitle("Example Expression Values from Four Samples") +
  theme(plot.title = element_text(hjust = 0.5))
```

## Re-centering the dataset

The first step of PCA is to find the central spot among all of our datapoints (samples) and use that as the origin to re-center the data. In order to do this, we need to do two steps:
...
*As with any data analysis, it is always a good idea to visualize your data to ensure that it appears to be behaving how you intended.* So let's visualize our re-centered data to ensure that the data appears centered around the origin. 

```{r plot_recentered_data}
# Plot the raw data after it has been shifted to have the center of the data align with the origin
ggplot(recentered_expression_tibble, aes(x = Gene_A, y = Gene_B, label = samples)) +
  geom_point(color = "cornflowerblue") +
  ...
  xlab("Gene A") +
  ylab("Gene B") +
  ggtitle("Example Re-centered Expression Values from Four Samples") +
  theme(plot.title = element_text(hjust = 0.5))
```
...
Where:
 ...
 - *x~i~* is an expression value for Gene A in sample *i*
 - *$\bar{x}$* is the mean expression value for Gene A
 - *y~i~* is an expression value for Gene B in sample *i*
...
::: callout-tip
# [**Exercise 1**](Aside_theory_of_PCA_Answer-key.qmd#exercise-1)
...
```{r plotting_PCA_hand}
# Create a tibble to hold the PC scores we found and also make the Samples into a column
...
ggplot(pc_scores_tibble, aes(x = PC_1, y = PC_2, label = samples)) +
  geom_point( color = "cornflowerblue") +
  geom_text(hjust = 0, vjust = -1) +
  theme_bw() +
  xlim(-50, 50) +
  ...
  ggtitle("PCA of Expression Values from Four Samples") +
  theme(plot.title = element_text(hjust = 0.5))
```

::: callout-tip
# [**Exercise 2**](Aside_theory_of_PCA_Answer-key.qmd#exercise-2)
...
In order to run `prcomp()`, we need to start from an expression matrix and move the samples to the rownames:

```{r prcomp_check}
# Run prcomp() on the expression tibble after moving the samples to be rownames
...
::: callout-tip
# [**Exercise 3**](Aside_theory_of_PCA_Answer-key#exercise-3)
...
When carrying out a PCA, it could be interesting the know how many the possible Principal Components axes exist. Technically, the answer isn't so simple and the metric to use is **the number of PCs ≤ minimum(number_of_genes, number_of_samples)**. But since the number of genes is almost always much larger than the number of samples, then the number of PCs in a bulk RNA-seq experiment is usually equal to the number of samples. For a deeper dive in how the number of PCs is determined and factors that influence them, consider [this lesson](Aside_Number_of_PCs.qmd).
