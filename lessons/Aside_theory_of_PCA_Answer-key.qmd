...
Create a plot of the Principal Components Analysis derived from `prcomp()`. Is it the same as the plot we derived except only reflected over the y-axis?

```{r plotting_PCA_prcomp}
# Create a tibble to hold the PC scores prcomp() found and also make the samples into a column
...
ggplot(prcomp_pc_scores_tibble, aes(x = PC1, y = PC2, label = samples)) +
...
Yes, it is the same plot just reflected of the y-axis.